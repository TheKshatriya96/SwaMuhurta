"""Build the V07 standalone workbook base.

V07 workbook base only:
...
try:
    ...
    raise SystemExit(
        ...
        f"{RECOMMENDED_PYTHON} {SCRIPT_DIR / 'v07_workbook_builder.py'}"
    ) from exc
from openpyxl import Workbook, load_workbook
from openpyxl.styles import Alignment, Font, PatternFill
from openpyxl.utils import get_column_letter

OUTPUT_WORKBOOK = BUILD_DIR / "MuhuratFinder_V07_Workbook.xlsx"
...
class FoundationConfig:
    """Workbook config for the V07 standalone workbook base.
...
def write_config_inputs(ws, config: FoundationConfig) -> None:
    ...
    ws["A1"] = "Muhurat Finder V07 Workbook"
    ws["A1"].font = Font(bold=True, size=14)
    ws["A2"] = (
        "V07 workbook base: raw ephemeris + event location + natal reference + helper columns. "
        "Parent-state columns are added in-place by the local V07 rule engine."
    )
...
def foundation_formula_map(row_number: int, header_map: dict[str, int], formula_col_map: dict[str, int]) -> dict[str, str]:
    ...
    formulas = {
        ...
        "CalculationLayerNote": '="V07 workbook base: raw ephemeris + location + natal + helper columns. Parent-state columns are applied later in the same workbook."',
    }
