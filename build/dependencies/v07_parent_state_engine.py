"""Apply the parent-state rule engine in-place on the V07 workbook."""
...
BASE_WORKBOOK = BUILD_DIR / "MuhuratFinder_V07_Workbook.xlsx"
...
def build_formulas(row_number: int, header_map: dict[str, int], v05_col_map: dict[str, int]) -> dict[str, str]:
    ...
    return {
        ...
        "V05CalculationNote": '="V07 parent-state rule engine. Scores are indicator-based support signals, not guarantees."',
    }
...
def main() -> None:
    ...
    if "INDICATORS" in wb.sheetnames:
        raise ValueError("INDICATORS sheet is not allowed in the V07 workbook.")
    ...
    if validation["forbidden_present"] or validation["duplicate_headers"] or not all(validation["required_row_checks"].values()):
        debug_path = DEBUG_DIR / "MuhuratFinder_V07_ParentStateEngine_DEBUG.xlsx"
